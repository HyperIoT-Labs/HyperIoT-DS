package it.acsoftware.hyperiot.spark.minimum
...
object Minimum {
  ...
  */
  ...
  val output = value
    ...
    .select(min(col("value")).as(outputName)) // compute minimum // TODO: change colName value with name of input
  ...
  output.collect().foreach(row => {
    ...
    val minimum = row(0).asInstanceOf[Double]  // TODO - framework issue: cast depending on output field type
    val columnFamily = "value"
    val column = outputName
    // TODO - framework issue: add as many columns as output fields
    transRec.addColumn(Bytes.toBytes(columnFamily), Bytes.toBytes(column), Bytes.toBytes(minimum.toString))
