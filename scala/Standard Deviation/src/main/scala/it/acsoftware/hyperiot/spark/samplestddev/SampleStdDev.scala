package it.acsoftware.hyperiot.spark.samplestddev
  ...
  def main(args: Array[String]) {
    ...
    */
    ...
    val output = value
      ...
      .select(stddev(col("value")).as(outputName)) // compute sample standard deviation // TODO: change colName value with name of input
    ...
    output.collect().foreach(row => {
      ...
      val sampleStDev = row(0).asInstanceOf[Double]  // TODO - framework issue: cast depending on output field type
      val columnFamily = "value"
      val column = outputName
      // TODO - framework issue: add as many columns as output fields
      transRec.addColumn(Bytes.toBytes(columnFamily), Bytes.toBytes(column), Bytes.toBytes(sampleStDev.toString))
